...
const industries = [
  {
    ...
    description:
      "Rugged telecom and communication infrastructure for upstream, midstream, and downstream oil & gas operations — engineered for hazardous zones, remote sites, and mission-critical reliability.",
  ...
  },
  {
    ...
    title: "Metro Rail",
    description:
      "End-to-end communication systems for metro rail networks including TETRA radio, CCTV, PIDS, and OFC backbone — ensuring safe, efficient, and passenger-centric transit operations.",
    points: ["TETRA & LTE-R Radio", "PIDS Systems", "Integrated CCTV & ACS"],
  ...
  <div className="max-w-7xl mx-auto px-6 lg:px-16">
    ...
    >
      ...
      <h2 className="text-4xl lg:text-5xl font-bold text-white leading-tight max-w-3xl">
        ...
        <span className="text-white">Critical Sectors</span>
      </h2>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      {industries.map((ind, i) => (
        ...
        >
          <Link
            href={ind.href}
            className="group relative flex flex-col h-full bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-white/20 transition-all duration-500 p-8"
          >
            {/* Top accent */}
            <span className="absolute top-0 left-0 w-0 h-[2px] bg-white/60 group-hover:w-full transition-all duration-700 ease-out" />
    ...
    >
      <Link
        ...
        className="inline-flex items-center gap-2 text-sm text-white/60 hover:text-white font-medium transition-colors duration-300 border border-white/10 hover:border-white/30 px-6 py-3 rounded-full"
      >
        View All Industries
